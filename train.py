...
def train_ner(train_buckets, dev_buckets, test_buckets, train_labels_raw, dev_labels_raw, test_labels_raw, model, crf_layer, config, params):
    print('training...')
    ...
    for epoch in range(config.maxIters):
        ...
        print('Dev...')
        dev_f1 = eval(dev_buckets, dev_labels_raw, model, crf_layer, config, params)
        if dev_f1 > best_f1:
            best_f1 = dev_f1
            print('\nTest...')
            test_f1 = eval(test_buckets, test_labels_raw, model, crf_layer, config, params)
        print('now, best fscore is: ', best_f1)


def eval(buckets, data_raw, model, crf_layer, config, params):
    model.eval()
    ...
    f_score = evaluation.eval_entity(data_raw[0], predict_path.transpose(0, 1).data.tolist(), params)
    return f_score
